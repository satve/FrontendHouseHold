...
namespace HouseHoldFront.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            var result = httpClient.PostAsync("http://localhost:58110/token",
            ...
            if (result.StatusCode == System.Net.HttpStatusCode.OK)
            {
                ...
                var authenticationInfo = JsonConvert.DeserializeObject<AuthenticationResponseModel>(jsonString);
        ...
        public async Task<ActionResult> Register(RegisterViewModel model, string returnUrl)
        {
            ...
            var result = httpClient.PostAsync("http://localhost:58110/api/account/register",
                formEncodedValues).Result;
            ...
            {
                var jsonString = result.Content.ReadAsStringAsync().Result;
                var errorResponse = JsonConvert.DeserializeObject<ErrorResponseModel>(jsonString);

                ModelState.AddModelError("", "Invalid register attempt.");

                if (errorResponse.ModelState.Any())
                {
                    foreach (var errorMessage in errorResponse.ModelState)
                    {
                        ModelState.AddModelError("", errorMessage.Messages[0]);
                    }
                }
                return View(model);
            }
        ...
        public async Task<ActionResult> ForgotPassword(ForgotPasswordViewModel model , string returnUrl)
        {
            ...
            var result = httpClient.PostAsync("http://localhost:58110/api/account/ForgotPassword",
                formEncodedValues).Result;
        ...
        public async Task<ActionResult> ResetPassword(ResetPasswordViewModel model, string returnUrl)
        {
            ...
            var parameters = new List<KeyValuePair<string, string>>();
            parameters.Add(new KeyValuePair<string, string>("email", model.Email));
            parameters.Add(new KeyValuePair<string, string>("NewPassword", model.Password));
            parameters.Add(new KeyValuePair<string, string>("Confirmpassword", model.ConfirmPassword));
            parameters.Add(new KeyValuePair<string, string>("Code", model.Code));
            var formEncodedValues = new FormUrlEncodedContent(parameters);

            var result = httpClient.PostAsync("http://localhost:58110/api/account/resetPassword",
                formEncodedValues).Result;
